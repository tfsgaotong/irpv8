...
function selectType(){
	var type=$("input[name='irpGoods.coverstate']:checked").val();
	if(type==20){
		$("#displayvalue").hide();
	}else{
		$("#displayvalue").show();
...
<input type="hidden" name="jsonFile" /><%-- 源文件名称 --%>
	...
	<input type="hidden" name="irpGoods.salesvolume" value="<s:property value='irpGoods.salesvolume'/>"> 
	...
	<tr>
		...
		<td  onchange="selectType()" bgcolor="#FFFFFF" class="main_bright" id="displaytype"><input name="irpGoods.coverstate" <s:if test="irpGoods.coverstate==20">checked="checked"</s:if> validType="length[2,60]"   type="radio" value="20" checked="checked" />是
