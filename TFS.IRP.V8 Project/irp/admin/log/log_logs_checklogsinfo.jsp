...
$(function(){
	 ...
	 if('${c_start_end}'=="logs_appoint_date"){
 			$('#start_time').datebox({
 ...
 //检索
   function log_CheckSearch(){
	   ...
	   jump('<%=rootPath%>log/log_checkSearch_action.action?c_v_log_type='+v_log_type+'&c_v_log_obj_oper='+v_log_obj_oper+'&c_v_log_oper_obj_id='+v_log_oper_obj_id+'&c_v_log_oper_user='+v_log_oper_user+'&c_v_Irp_type_oper='+v_Irp_type_oper+'&c_start_end='+start_end+'&c_v_Irp_oper_result='+v_Irp_oper_result+'&c_date_start_time='+date_start_time+'&c_date_end_time='+date_end_time+'&pageNum=1&orderField');
   }
 ...
 //分页
	function page(_nPageNum){
	   ...
	   var v_Irp_oper_result=$('#Irp_oper_result').combobox('getValue');
		...
		jump('<%=rootPath%>log/log_checkSearch_action.action?'+queryString+'&c_v_log_type='+v_log_type+'&c_v_log_obj_oper='+v_log_obj_oper+'&c_v_log_oper_obj_id='+v_log_oper_obj_id+'&c_v_log_oper_user='+v_log_oper_user+'&c_v_Irp_type_oper='+v_Irp_type_oper+'&c_start_end='+start_end+'&c_v_Irp_oper_result='+v_Irp_oper_result+'&c_date_start_time='+date_start_time+'&c_date_end_time='+date_end_time);
	}
	...
	function logRefresh(){
		...
		var date_end_time;                   
		var v_log_type=0;
		var v_log_obj_oper=0;
		var v_log_oper_obj_id=-1;
		var v_log_oper_user="";
		var v_Irp_type_oper=0;
		var start_end="logs_month";
		var v_Irp_oper_result=0;
		$('body').layout('panel','center').panel('refresh','<%=rootPath%>log/log_checkSearch_action.action?c_v_log_type='+v_log_type+'&c_v_log_obj_oper='+v_log_obj_oper+'&c_v_log_oper_obj_id='+v_log_oper_obj_id+'&c_v_log_oper_user='+v_log_oper_user+'&c_v_Irp_type_oper='+v_Irp_type_oper+'&c_start_end='+start_end+'&c_v_Irp_oper_result='+v_Irp_oper_result+'&c_date_start_time='+date_start_time+'&c_date_end_time='+date_end_time);
	}
//引入扩展验证
	...
	<table width="100%" border="0" align="center" cellpadding="10"
			...
			<td colspan="8" style="padding-left: 10px;">
				<form id="check_logs" method="post">
					日志类型 :<select id="_log_type" name="c_v_log_type">
						<option value="0"
							<s:if test="c_v_log_type==0">selected="true"</s:if>>全部</option>
						<option value="1"
							<s:if test="c_v_log_type==1">selected="true"</s:if>>错误</option>
						<option value="2"
							<s:if test="c_v_log_type==2">selected="true"</s:if>>警告</option>
						<option value="3"
							<s:if test="c_v_log_type==3">selected="true"</s:if>>信息</option>
						<option value="4"
							<s:if test="c_v_log_type==4">selected="true"</s:if>>调试</option>
					</select>
					&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;
					 操作对象:<select id="_log_obj_oper" panelHeight="150"
						...
						<s:iterator value="listIrpTableid">
							<option value="<s:property value='TABLEOBJTYPE' />"<s:if test="TABLEOBJTYPE==c_v_log_obj_oper">selected="true"</s:if>><s:property value='TABLEOBJTYPE' /></option>
						</s:iterator>
					</select>
					&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp; 
					操作对象id:<input name="c_v_log_oper_obj_id"
						...
						<s:if test="c_v_log_oper_obj_id!=-1">value="<s:property value="c_v_log_oper_obj_id" />"</s:if>>
					&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;	
					操作用户: <input name="c_v_log_oper_user" id="log_oper_user"
						class="easyui-validatebox"
						
						value="<s:property value="c_v_log_oper_user" />"><br/>

					操作类型:<select id="_Irp_type_oper" name="c_v_Irp_type_oper">
						<option value="0" selected="selected">全部</option>
						<s:iterator value="irpOpertype">
							<option value="<s:property value='opername' />"<s:if test="opername==c_v_Irp_type_oper">selected="true"</s:if>><s:property value="opername" /></option>
						</s:iterator>
					</select>
					&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;
					请选择时间段:<select id="Irp_logtime" name="c_start_end"
						onchange="ck_date()">
						<option value="logs_week" id="_logs_week"
							<s:if test="c_start_end=='logs_week'">selected="true"</s:if>>本周</option>
						<option value="logs_month" id="_logs_month"
							<s:if test="c_start_end=='logs_month'">selected="true"</s:if>>本月</option>
						<option value="logs_quarter" id="_logs_quarter"
							<s:if test="c_start_end=='logs_quarter'">selected="true"</s:if>>本季</option>
						<option value="logs_appoint_date" id="_logs_appoint_date"
							<s:if test="c_start_end=='logs_appoint_date'">selected="true"</s:if>>指定</option>
					...
					&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;
						 操作结果:<select id="Irp_oper_result"
						panelHeight="100" name="c_v_Irp_oper_result">
						<option value="0"
							<s:if test="c_v_Irp_oper_result==0">selected="true"</s:if>>全部</option>
						<option value="1"
							<s:if test="c_v_Irp_oper_result==1">selected="true"</s:if>>成功</option>
						<option value="2"
							<s:if test="c_v_Irp_oper_result==2">selected="true"</s:if>>失败</option>
		...
		<tr>
			<td align="center" bgcolor="#F5FAFE" class="main_bright" width="5%"><a
				href="javascript:void(0)" onclick="checkAll()">序号</a></td>
			...
			<td align="center" bgcolor="#F5FAFE" class="main_bright"><a
				...
				onclick="orderBy('logtype','<s:if test="orderField=='logtype'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">日志类型<s:if
					...
					</s:if> <s:elseif test="orderField=='logtype'&&orderBy=='asc'">
			...
			<td align="center" bgcolor="#F5FAFE" class="main_bright"><a
				...
				onclick="orderBy('logobjname','<s:if test="orderField=='logobjname'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">操作对象<s:if
						...
						<img src="<%=rootPath%>admin/images/spinner_arrow_down.gif" />
					</s:if> <s:elseif test="orderField=='logobjname'&&orderBy=='asc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_up.gif" />
					</s:elseif> </a></td>
			<td align="center" bgcolor="#F5FAFE" class="main_bright"><a
				href="javascript:void(0)"
				onclick="orderBy('logoptype','<s:if test="orderField=='logoptype'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">操作类型<s:if
						...
						<img src="<%=rootPath%>admin/images/spinner_arrow_down.gif" />
					</s:if> <s:elseif test="orderField=='logoptype'&&orderBy=='asc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_up.gif" />
					</s:elseif> </a></td>
			<td align="center" bgcolor="#F5FAFE" class="main_bright"><a
				href="javascript:void(0)"
				onclick="orderBy('loguser','<s:if test="orderField=='loguser'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">操作用户<s:if
						...
						<img src="<%=rootPath%>admin/images/spinner_arrow_down.gif" />
					</s:if> <s:elseif test="orderField=='loguser'&&orderBy=='asc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_up.gif" />
					</s:elseif> </a></td>
			<td align="center" bgcolor="#F5FAFE" class="main_bright"><a
				href="javascript:void(0)"
				onclick="orderBy('logoptime','<s:if test="orderField=='logoptime'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">操作时间<s:if
						...
						<img src="<%=rootPath%>admin/images/spinner_arrow_down.gif" />
					</s:if> <s:elseif test="orderField=='logoptime'&&orderBy=='asc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_up.gif" />
					</s:elseif> </a></td>
			<td align="center" bgcolor="#F5FAFE" class="main_bright" width="7%"><a
				...
				onclick="orderBy('logresult','<s:if test="orderField=='logresult'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">操作结果<s:if
						...
						<img src="<%=rootPath%>admin/images/spinner_arrow_down.gif" />
					</s:if> <s:elseif test="orderField=='logresult'&&orderBy=='asc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_up.gif" />
					</s:elseif> </a></td>
		</tr>
		<s:if test="irpLogs.size()!=0">
			<s:iterator var="irpLogs_type" value="irpLogs"
				...
				<tr>
					...
					<td align="center" bgcolor="#F5FAFE" class="main_bright" width="5%"><a
						href="javascript:void(0)"
						onclick="checkLog('<s:property value="logid" />')">查看</a></td>
					<s:if test="logtype==3">
						<td align="center" bgcolor="#F5FAFE" class="main_bright" width="5%">信息</td>
					</s:if>
					<s:if test="logtype==1">
						<td align="center" bgcolor="#F5FAFE" class="main_bright" width="5%">错误</td>
					</s:if>
					<s:if test="logtype==2">
						<td align="center" bgcolor="#F5FAFE" class="main_bright" width="5%">警告</td>
					</s:if>
					<s:if test="logtype==4">
						<td align="center" bgcolor="#F5FAFE" class="main_bright" width="5%">调试</td>
					</s:if>
					...
					<td align="center" bgcolor="#F5FAFE" class="main_bright" width="15%"><s:date
							name="logoptime" format="yyyy-MM-dd HH:mm:ss" />
					</td>
