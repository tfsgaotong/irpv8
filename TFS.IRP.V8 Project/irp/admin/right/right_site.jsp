...
$(function(){
	...
	$('#siteRight').datagrid({
		url:'<%=rootPath%>right/find_datagrid_node.action?type=<s:property value="type" />&objType=<s:property value="objType" />&objId=<s:property value="objId" />',
		fit:true,
		fitColumns:true,
		idField:'operid',
		singleSelect:true,
		...
		onDblClickRow:function(rowIndex, rowData){
			if(editIndex != rowIndex){
				if(editIndex!=undefined){
					$('#siteRight').datagrid('endEdit', editIndex);
				}
				editIndex = rowIndex;
				$('#siteRight').datagrid('beginEdit', editIndex);
			}
		}
	});
});
</script>
<table id="siteRight"></table>
</body>
</html>
