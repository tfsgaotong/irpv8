...
function import_page(_nPageNum){
	...
	import_dialogJump("<%=rootPath %>set/selectspecialuser.action?"+queryString);
}
//排序
		 ...
		 for(var i = 0;i<messageuserinfouserarray.length;i++){
			 if(messageuserinfouserarray[i]!=null){
					$("input[name='userid']").each(function(){
						if($(this).attr('id')==messageuserinfouserarray[i]){
							$("#"+messageuserinfouserarray[i]).attr("checked",true);
						}
			 		});
			 }
		 }
...
<table id="importUserList" width="100%" border="0" align="center" cellpadding="10" cellspacing="1" bgcolor="#cad9ea">
    ...
    <tr>
	    <td align="center" bgcolor="#F5FAFE"  class="main_bleft"><input id="<s:property value="username" />" type="checkbox" <s:if test='@org.apache.commons.lang.ArrayUtils@contains(#arr,""+userid)'>checked="true"</s:if> onclick="import_checkClick(this)" name="userid" value="<s:property value="userid" />" /> <s:property value="(pageNum-1)*pageSize+#listStat.count"/></td>
