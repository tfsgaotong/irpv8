...
function page(_nPageNum){
	...
	$('#loadUserContributeDetaildiv').panel('refresh',"<%=rootPath%>set/loadUserContributeDetail.action?"+queryString);
}
/**
*排序
*/
function orderBy(_sFiled,_sOrderBy){
	...
	var queryString = $('#pageForm').serialize();
	$('#loadUserContributeDetaildiv').panel('refresh',"<%=rootPath%>set/loadUserContributeDetail.action?"+queryString+"&orderFieldtwo="+_sFiled);
}
...
function searchDate(_formNum){
	...
	var queryString = $('#pageForm').serialize();
		$('#loadUserContributeDetaildiv').panel('refresh',"<%=rootPath%>set/loadUserContributeDetail.action?"+queryString+"&datanumid="+_formNum+"&date_start_time="+date_start_time+"&date_end_time="+date_end_time);
...
<table width="100%" border="0" align="center" cellpadding="10" cellspacing="1" bgcolor="#cad9ea">
 ...
 <tr>
  ...
  <td align="center" bgcolor="#F5FAFE" class="main_bright"><a href="javascript:void(0)" onclick="orderBy('valueckey','<s:if test="orderFieldtwo=='valueckey'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">类型<s:if test="orderFieldtwo=='valueckey'&&orderBy=='desc'"><img src="<%=rootPath %>admin/images/spinner_arrow_down.gif" /></s:if><s:elseif test="orderFieldtwo=='valueckey'&&orderBy=='asc'"><img src="<%=rootPath %>admin/images/spinner_arrow_up.gif" /></s:elseif></a></td>
  <td align="center" bgcolor="#F5FAFE" class="main_bright"><a href="javascript:void(0)" onclick="">描述</a></td>
  <td align="center" bgcolor="#F5FAFE" class="main_bright"><a href="javascript:void(0)" onclick="orderBy('score','<s:if test="orderFieldtwo=='score'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">分数<s:if test="orderFieldtwo=='score'&&orderBy=='desc'"><img src="<%=rootPath %>admin/images/spinner_arrow_down.gif" /></s:if><s:elseif test="orderFieldtwo=='score'&&orderBy=='asc'"><img src="<%=rootPath %>admin/images/spinner_arrow_up.gif" /></s:elseif></a></td>
  <td align="center" bgcolor="#F5FAFE" class="main_bright"><a href="javascript:void(0)" onclick="orderBy('experience','<s:if test="orderFieldtwo=='experience'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">经验<s:if test="orderFieldtwo=='experience'&&orderBy=='desc'"><img src="<%=rootPath %>admin/images/spinner_arrow_down.gif" /></s:if><s:elseif test="orderFieldtwo=='experience'&&orderBy=='asc'"><img src="<%=rootPath %>admin/images/spinner_arrow_up.gif" /></s:elseif></a></td>
 ...
 <s:iterator value="irpUserValueLink" status="irpUserValueLinkStatus" >

 <tr>
  <td align="center" bgcolor="#F5FAFE" class="main_bright" width="5%"><input type="checkbox" name="irpvalueconfigdetailid">&nbsp;<s:property value="(pageNum-1)*pageSize+#irpUserValueLinkStatus.count"/></td>
  <td align="center" bgcolor="#F5FAFE" class="main_bright" width="20%"><s:property value="valueckey" /></td>
  <td align="center" bgcolor="#F5FAFE" class="main_bright" width="25%"><s:property value="getIrpValueConfig(valueckey).valuedesc" /></td>
  <td align="center" bgcolor="#F5FAFE" class="main_bright" width="5%"><s:property value="score" /></td>
  <td align="center" bgcolor="#F5FAFE" class="main_bright" width="5%"><s:property value="experience" /></td>
  <td align="center" bgcolor="#F5FAFE" class="main_bright" width="20%"><s:date name="crtime" format="yyyy-MM-dd HH:mm:ss" /></td>
 </tr>
