 ...
 */
 ...
 function addEdit(){
 	...
 	$('#addconfig').dialog({
 		modal:true,
 		cache:false,
 		href:'<%=rootPath%>admin/set/set_data_edit.jsp',
 		height:200,
 		width:400,
 		title:'添加数据管理配置',
 		resizable:true,
 		buttons:[{
 			...
 			handler:function(){
 			 $('#editconfig').form('submit',{
 				 url:'<%=rootPath%>set/adddataSetting.action',
 				 onSubmit:function(){
  ...
  function deleteEdit(_ckey,_configid){
 	$.messager.confirm("删除配置","您确定删除名称为"+_ckey+"的配置吗",function(r){
 		if(r){
 			...
 			$.ajax({
 				type:'post',
 				url:'<%=rootPath%>set/deletedataSetting.action?configid='+_configid,
	 ...
	 */
		...
		*/
		function page(_nPageNum){
			...
			$('#datasetting').panel('refresh',"<%=rootPath%>set/dataSetting.action?"+queryString);
		}
		/**
		*排序
		*/
