 ...
 */
	...
	*/
	function page(_nPageNum){
		...
		$('#sysemailtab').panel('refresh',"<%=rootPath%>set/setemaillist.action?"+queryString);
	}
	/**
	*排序
	*/
	...
	<table width="100%" border="0" align="center" cellpadding="10"
		...
		<tr class="list_th" style="position: relative;">
			<td colspan="3" style="padding-left: 10px;"><a
				href="javascript:void(0)" onclick="deleteAll()">删除</a> <a
				href="javascript:void(0)" onclick="refreshCata()">刷新</a></td>
			<td colspan="3" align="right" style="padding-right: 2px;"><input
		...
		<tr>
			...
			<td align="center" width="120" bgcolor="#F5FAFE" class="main_bright"><a
				href="javascript:void(0)"
				onclick="orderBy('ckey','<s:if test="orderField=='ckey'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">名称<s:if
						test="orderField=='ckey'&&orderBy=='desc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_down.gif" />
					</s:if>
					<s:elseif test="orderField=='ckey'&&orderBy=='asc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_up.gif" />
					</s:elseif></a></td>
			<td align="center" width="180" bgcolor="#F5FAFE" class="main_bright"><a
				href="javascript:void(0)"
				onclick="orderBy('cvalue','<s:if test="orderField=='cvalue'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">内容<s:if
						test="orderField=='cvalue'&&orderBy=='desc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_down.gif" />
					</s:if>
					<s:elseif test="orderField=='cvalue'&&orderBy=='asc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_up.gif" />
					</s:elseif></a></td>
			<td align="center" bgcolor="#F5FAFE" class="main_bright"><a
				href="javascript:void(0)"
				onclick="orderBy('cdesc','<s:if test="orderField=='cdesc'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">说明<s:if
						test="orderField=='cdesc'&&orderBy=='desc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_down.gif" />
					</s:if>
					<s:elseif test="orderField=='cdesc'&&orderBy=='asc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_up.gif" />
					</s:elseif></a></td>
			<td align="center" width="70" bgcolor="#F5FAFE" class="main_bright"><a
				href="javascript:void(0)"
				onclick="orderBy('modified','<s:if test="orderField=='modified'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">是否可修改<s:if
					...
					<s:elseif test="orderField=='modified'&&orderBy=='asc'">
						<img src="<%=rootPath%>admin/images/spinner_arrow_up.gif" />
					</s:elseif></a></td>
		...
		<s:iterator value="irpConfig" status="irpconfigstatus">
			<tr>
				<td align="left" bgcolor="#F5FAFE" class="main_bright"><input
					...
					value="<s:property value='configid' />">&nbsp;<s:property
						value="(pageNum-1)*pageSize+#irpconfigstatus.count" /></td>
				...
				<td align="center" bgcolor="#F5FAFE" class="main_bright">
					<img border="0" src="images/icons/pencil.png" title="修改"
					style="cursor:pointer; margin: 0 5px;"
					onclick="updateEdit(<s:property value="configid" />)" /> <img
					border="0" src="images/icons/cancel.png" title="删除"
					style="cursor:pointer; margin: 0 3px;"
					onclick="deleteEdit('<s:property value="ckey" />',<s:property value="configid" />)" />
				</td>
			</tr>
		</s:iterator>
		<tr bgcolor="#FFFFFF">
			<td colspan="6" align="right" class="page"><s:property
