		...
		buttons:[{
			...
			handler:function(){
				$('#editconfig').form('submit',{
					url:'<%=rootPath%>set/updatecatelogue.action?configid='+_configid,
					onSubmit:function(){
...
function addEdit(){
	...
	$('#addconfig').dialog({
		modal:true,
		cache:false,
		href:'<%=rootPath%>set/catalogueAddform.action',
		height:200,
		width:400,
		title:'添加存放目录',
		resizable:true,
		buttons:[{
			...
			handler:function(){
			 $('#editconfig').form('submit',{
				 url:'<%=rootPath%>set/addcatelogue.action',
				 onSubmit:function(){
						var isValid = $(this).form('validate');
	    				if (isValid){
	    					$.messager.progress({
 ...
 */
	...
	*/
	function page(_nPageNum){
		...
		$('#cataloguelist').panel('refresh',"<%=rootPath%>set/knowledgecatalogue.action?"+queryString);
	}
	/**
	*排序
	*/
