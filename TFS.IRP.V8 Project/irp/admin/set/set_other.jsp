 		...
 		buttons:[{
 			...
 			handler:function(){
 			 $('#editconfig').form('submit',{
 				 url:'<%=rootPath%>set/addOtherSet.action',
 				 onSubmit:function(){
  ...
  function deleteEdit(_ckey,_configid){
 	$.messager.confirm("删除配置","您确定删除名称为"+_ckey+"的配置吗",function(r){
 		if(r){
 			...
 			$.ajax({
 				type:'post',
 				url:'<%=rootPath%>set/deleteOtherSet.action?configid='+_configid,
	 ...
	 */
		...
		*/
		function page(_nPageNum){
			...
			$('#frontconfitem').panel('refresh',"<%=rootPath%>set/otherSet.action?"+queryString);
		}
		/**
		*排序
		*/
