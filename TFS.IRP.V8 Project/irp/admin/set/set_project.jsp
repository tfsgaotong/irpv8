...
$(function(){
	$('#listSearchText').searchbox({
	    ...
	    searcher:function(value,name){   
	    	...
	    	$('#projectmenu').panel('refresh',"<%=rootPath%>set/project_menu.action?"+queryString);
	    },   
	    menu:'#listSearchType',   
	    prompt:'请输入检索词'  
	});
 ...
 */
 ...
 function updateEdit(_configid){
 	...
 	$('#updatereportconfig').dialog({
 		modal:true,
 		href:'<%=rootPath%>set/updateProjectInformType.action?informtypeid='+_configid,
 		height:260,
 		width:400,
 		title:'修改常用菜单',
 		resizable:true,
 ...
 function addEdit(){
 	...
 	$('#addconfig').dialog({
 		modal:true,
 		cache:false,
 		href:'<%=rootPath%>admin/set/set_project_edit.jsp',
 		height:200,
 		width:400,
 		title:'添加圈子阶段配置',
 		resizable:true,
...
<table width="100%" border="0" align="center" cellpadding="10"
		...
		<tr>
		 ...
		 <td align="center" bgcolor="#F5FAFE" class="main_bright" width="5%"><s:if test="informtype==10">微知</s:if><s:if test="informtype==30">常用</s:if><s:else>知识</s:else> </td>
