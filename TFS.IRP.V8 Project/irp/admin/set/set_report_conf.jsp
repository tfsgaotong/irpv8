...
$(function(){
	$('#listSearchText').searchbox({
	    ...
	    searcher:function(value,name){   
	    	...
	    	$('#frontconfreporttype').panel('refresh',"<%=rootPath%>set/reportkindset.action?"+queryString);
	    },   
	    menu:'#listSearchType',   
	    prompt:'请输入检索词'  
	});
...
function refreshCata(){
		$('#frontconfreporttype').panel('refresh');
}
/**
 * 增加举报种类配置
 */
function addEdit(){
 	...
 	$('#addreportconfig').dialog({
 		modal:true,
 		cache:false,
 		href:'<%=rootPath%>admin/set/set_report_conf_edit.jsp',
 		height:200,
 		width:400,
 		title:'添加举报种类配置',
 		resizable:true,
 ...
 */
 ...
 function updateEdit(_configid){
 	...
 	$('#updatereportconfig').dialog({
 		modal:true,
 		href:'<%=rootPath%>set/updateInformType.action?informtypeid='+_configid,
 		height:200,
 		width:400,
 		title:'修改举报种类配置',
 		resizable:true,
...
<table width="100%" border="0" align="center" cellpadding="10"
		...
		<tr>
		 ...
		 <td align="center" bgcolor="#F5FAFE" class="main_bright"><s:if test="informtype==10">微知</s:if><s:elseif test="informtype==30">常用</s:elseif> <s:else>知识</s:else> </td>
