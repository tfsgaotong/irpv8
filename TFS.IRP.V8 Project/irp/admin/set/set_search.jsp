 		...
 		buttons:[{
 			...
 			handler:function(){
 			 $('#editconfig').form('submit',{
 				 url:'<%=rootPath%>set/addSearchSet.action',
 				 onSubmit:function(){
  ...
  function updateEdit(_configid){
  	...
  	$('#updateconfig').dialog({
  		...
  		buttons:[{
  			...
  			handler:function(){
  				$('#editconfig').form('submit',{
  					url:'<%=rootPath%>set/updateSearchConfigOther.action?configid='+_configid,
  					onSubmit:function(){
  						var isValid = $(this).form('validate');
  	    				if (isValid){
  	    					$.messager.progress({
	 ...
	 */
		...
		*/
		function page(_nPageNum){
			...
			$('#solrsearchconf').panel('refresh',"<%=rootPath%>set/searchset.action?"+queryString);
		}
		/**
		*排序
		*/
