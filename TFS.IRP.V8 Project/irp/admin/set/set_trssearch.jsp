 ...
 */
 ...
 function addEdit(){
 	...
 	$('#addconfig').dialog({
 		modal:true,
 		cache:false,
 		href:'<%=rootPath%>admin/set/set_other_edit.jsp',
 		height:220,
 		width:400,
 		title:'添加TRS检索配置',
 		resizable:true,
 		buttons:[{
 			...
 			handler:function(){
 			 $('#editconfig').form('submit',{
 				 url:'<%=rootPath%>menu/addtrsSearchSet.action',
 				 onSubmit:function(){
  ...
  function deleteEdit(_ckey,_configid){
 	$.messager.confirm("删除配置","您确定删除名称为"+_ckey+"的配置吗",function(r){
 		if(r){
 			...
 			$.ajax({
 				type:'post',
 				url:'<%=rootPath%>set/deleteSearchOtherSet.action?configid='+_configid,
  ...
  function updateEdit(_configid){
  	...
  	$('#updateconfig').dialog({
  		...
  		buttons:[{
  			...
  			handler:function(){
  				$('#editconfig').form('submit',{
  					url:'<%=rootPath%>menu/updatetrsSearchConfigOther.action?configid='+_configid,
  					onSubmit:function(){
  						var isValid = $(this).form('validate');
  	    				if (isValid){
  	    					$.messager.progress({
	 ...
	 $("input[name='irpconfigcatach']:checked").each(
			 ...
			 $.messager.confirm("操作提示","您确定要删除选中的"+_configcatalength+"条数据吗",function(r){
				 if(r){
				   ...
				   $.ajax({
				    type:'post',
				    url:'<%=rootPath  %>set/deleteSearchAll.action?configidall='+_configid,
	 ...
	 */
		...
		*/
		function page(_nPageNum){
			...
			$('#trssearchconf').panel('refresh',"<%=rootPath%>menu/trssearchset.action?"+queryString);
		}
		/**
		*排序
		*/
...
<table width="100%" border="0" align="center" cellpadding="10"
		...
		<tr class="list_th" style="position: relative;">
		 <td colspan="3" style="padding-left: 10px;">
		  ...
		  <td colspan="3" align="right" style="padding-right: 2px;"><input id='listSearchText' value='<s:property value="searchWord"/>'/></td>
		</tr>
		<tr>
