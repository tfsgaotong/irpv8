 ...
 var dialogDiv1 = document.createElement("div");
  ...
  <form   id="addchannelfrm" method="post">
	...
	<input type="hidden" name="irpChannel.chnltype" value="<s:property value='chnlType'/>">
	     ...
	     %>
	       <s:if test="irpChannel.parentid==3">
	       <tr>
	       		<td width="30%" align="right" bgcolor="#f5fafe" class="main_bleft">地图类型：</td>
		        <td bgcolor="#FFFFFF" class="main_bright">
			    	<select name="irpChannel.maptype" style="width: 100px;">
			    		<option value="410" <s:if test="maptype==410">selected="selected"</s:if>>一维知识地图</option>
			    		<option value="420" <s:if test="maptype==420">selected="selected"</s:if>>二维知识地图</option>
			    		<option value="430" <s:if test="maptype==430">selected="selected"</s:if>>三维知识地图</option>
			    		<option value="440" <s:if test="maptype==440">selected="selected"</s:if>>其他知识地图</option>
			    	</select>
			    </td>
	       </tr>  
	     </s:if>
	     <s:else>
	     	<input type="hidden" name="irpChannel.maptype" value="<s:property value='maptype'/>">
	     </s:else>
	      <tr>
