	  ...
	  var tableStr = "<table class='ellipsis'  id=updateatttab width=100%  border=0 align=center cellpadding=10 cellspacing=1 bgcolor=#cad9ea >"; 
			  ...
			  for(var i=0 ;i<len ; i++){   
			 	if(addJsonFileList[i].editversions=="1"){//选择的文件
			 	 tableStr = tableStr + "<tr><td bgcolor=#F5FAFE class=main_bleft>"+ addJsonFileList[i].attdesc +"</td><td  bgcolor=#F5FAFE class=main_bleft>"+addJsonFileList[i].attlinkalt+"</td><td align=center bgcolor=#F5FAFE class=main_bleft><input type='radio' value=1 name='editversions' checked id='"+addJsonFileList[i].attfile+"'></td>"; 
			    }
			 	 if(addJsonFileList[i].editversions=="0"){//一种就是图片
						tableStr =tableStr + "<tr><td onmouseout=\"hidepic()\" onmouseover=\"showpic('"+addJsonFileList[i].attfile+"')\" id=\"lookimg\" style=\"cursor:pointer; margin: 0 3px;\" name="+addJsonFileList[i].attfile+"   bgcolor=#F5FAFE class=main_bleft>"+ addJsonFileList[i].attdesc +"</td><td  bgcolor=#F5FAFE   class=main_bleft>"+addJsonFileList[i].attlinkalt+"</td>";
						tableStr +="<td align=center bgcolor=#F5FAFE class=main_bleft><input type='radio' value=1 name='editversions'  id='"+addJsonFileList[i].attfile+"'></td>";
				}
			    if(addJsonFileList[i].editversions=="2"){//一种就不是图片的附件
			        tableStr = tableStr + "<tr><td onmouseout=\"hidepic()\" onmouseover=\"showpic('"+addJsonFileList[i].attfile+"')\" id=\"lookimg\" style=\"cursor:pointer; margin: 0 3px;\" name="+addJsonFileList[i].attfile+" bgcolor=#F5FAFE class=main_bleft>"+ addJsonFileList[i].attdesc +"</td><td  bgcolor=#F5FAFE  class=main_bleft>"+addJsonFileList[i].attlinkalt+"</td>";
				 	tableStr +="<td align=center bgcolor=#F5FAFE class=main_bleft></td>";
			    }
			    tableStr+="<td align='center' bgcolor=#F5FAFE class=main_bleft><input  onkeyup='updateorderby(this)' onbeforepaste='updateorderby(this)'  value='"+addJsonFileList[i].attflag+"'></td>";
    	    ...
    	    $("#uploadify").uploadify({
    	        ...
    	        'onFallback':function(){
    	         ...
    	         'onUploadSuccess':function(file, data, response){ 
    	        	...
    	        	//当table加一行 
   	   	            var rowtr="<tr class='ui-state-default'>";
   	   	      		 //源文件名
    	   	        ...
    	   	        var kValue=2;
		   	         ...
		   	         $.ajax({
 		   	         ...
 		   	         var  tablerowAmount=$('#updateatttab').find('tr');
 	   	         	 rowtr=rowtr+"<td bgcolor='#F5FAFE' class='main_bleft' align='center'><input type='text' style='width:30px;' onkeyup='updateorderby(this)' onbeforepaste='updateorderby(this)' value='"+tablerowAmount.length+"'></td>";
 	   	       		//操作
