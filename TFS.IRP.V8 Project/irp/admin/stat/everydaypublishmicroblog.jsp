...
function showData(step){
		$('#container').highcharts({
			 colors:['#f28f43'] ,////不同组数据的显示背景色，循环引用 
			chart:{
				type:'spline'//区域图
			},
			title:{
				...
				formatter:function(){//显示的数据 
					var timeLimit='<s:property value="timeLimit"/>';
				 	if(timeLimit=="thismonth"){
				 		return "本月[ "+this.x+" ]号发布了[ "+this.y+" ]条 "+this.series.name+" 微知.";
				 	}else if(timeLimit=="thisweek"){
				 		return "[ 星期"+this.x+" ]发布了[ "+this.y+" ]条"+this.series.name+" 微知.";
				 	}else if(timeLimit=="thisquarter"){
				 		return "本季度第[ "+this.x+" ]天发布了[ "+this.y+" ]条"+this.series.name+" 微知.";
				 	}else {
				 		return this.x+"发布了[ "+this.y+" ]条"+this.series.name+" 微知.";
				 	}
				} 
			},
			plotOptions:{
...
</span>		<a id="log_search"  class="easyui-linkbutton l-btn" onclick="searchDate('<%=rootPath %>site/everydaypublishmicroblog.action')" data-options="iconCls:'icon-search'" href="javascript:void(0);">
		查询</a>
		<span>
