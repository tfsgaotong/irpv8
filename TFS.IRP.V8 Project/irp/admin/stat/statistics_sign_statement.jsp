	            ...
	            events: {  
	              load: function() {
	            	  ...
	            	  if(amountStr!=""){
							...
							var again=arrays[3];
							var data = [];  
							data.push(['正常', window.parseInt(normal)]);  
							data.push(['迟到', window.parseInt(late)]);  
							data.push(['早退', window.parseInt(early)]); 
							data.push(['补签', window.parseInt(again)]);
							series.setData(data); 
	            	  }
...
</span>		<a id="log_search"  class="easyui-linkbutton l-btn" onclick="searchDate('<%=rootPath %>sign/checkByBing.action')" data-options="iconCls:'icon-search'" href="javascript:void(0);">
		查询</a>
</div>
