...
<script type="text/javascript">
		...
		$('#editUser').dialog({   
		    ...
		    buttons: [{
		    	...
		    	handler: function() {
					...
					$('#addUserForm').find("input:hidden[name='experttype']").val(recommend);
	...
	function createUser() {
		...
		$('#createUser').dialog({   
		    modal:true,
		    href:'<%=rootPath %>user/create_user.action',
		    title:'新建用户',
		    width:500,
		    height:531,
		    resizable:true,
		    	...
		    	handler: function() {
					...
					$('#addUserForm').form('submit',{
		    			...
		    			success:function(data){
		    				...
		    				$.messager.progress('close');
		    				if(data==1){
		    					$.messager.alert("提示信息","新建用户成功","info");
		    					$('body').layout('panel','center').panel('refresh');
		    				}else{
		    					$.messager.alert("提示信息","添加用户信息失败！","error");
		    				}
		    			}
		    		});
		    	} 
	...
	function importExcel(){
		...
		$('#importExcel').dialog({   
		    modal:true,
		    href:'<%=rootPath%>file/to_import_user_excel.action',
		    title:'导入Excel',
		    width:450,
		    height:300,
		    resizable:true,
...
<table width="100%"  border="0" align="center" cellpadding="10" cellspacing="1" bgcolor="#cad9ea">
	<tr class="list_th" style="position: relative;">
   	  	<td colspan="4" align="left" style="padding-left: 10px;" nowrap="nowrap">
   	  		<a href="javascript:void(0)" onclick="createUser()">新建用户</a>
   	  		<a href="javascript:void(0)" onclick="forbidUsers()">停用用户</a>
   	  		<a href="javascript:void(0)" onclick="updatePwd()">重置密码</a>
   	  		<a href="javascript:void(0)" onclick="importExcel()">导入Excel</a>
    ...
    <tr>
	    ...
	    <td bgcolor="#F5FAFE" align="center" class="main_bright" align="center"><s:date name="regtime" format="yyyy-MM-dd HH:mm:ss" /></td>
