...
<style type="text/css">
 ...
 table {   border-collapse:    separate;   border-spacing:  1px 3px;  } 
  ...
  <body>
   ...
   //全局变量
    ...
    function addversion(){
    	...
    	if(versionname ==''){
	    	...
	    	urlStr="<%=rootPath%>bugconfig/addversion.action?proid="+pid+"&projectId="+pid+"&versionname="+versionname;
	    	var result = $.ajax({
				url:urlStr,
				type:"post",
				dataType: "json", 
    ...
    function addmodul(){
    	...
    	if(modulname ==''){
	    	...
	    	urlStr="<%=rootPath%>bugconfig/addmodul.action?proid="+pid+"&projectId="+pid+"&modulname="+modulname;
	    	var result = $.ajax({
				url:urlStr,
				type:"post",
				//dataType: "json", 
	...
	function editsaveversion(objj){
    	...
    	var bugconfigid=$(t_this).parent().parent().find("td:lt(1)").find('input[name=bugconfigid]').val();
    	var urlStr="<%=rootPath%>bugconfig/editsaveversion.action?versionname="+versionname+"&projectId="+projectId+"&bugconfigid="+bugconfigid;
    	var result = $.ajax({
			url:urlStr,
			type:"post",
			dataType: "json", 
	...
	function editsavemodul(objj){
    	...
    	var bugconfigid=$(t_this).parent().parent().find("td:lt(1)").find('input[name=bugconfigid]').val();
    	var urlStr="<%=rootPath%>bugconfig/editsavemodul.action?modulname="+modulname+"&projectId="+projectId+"&bugconfigid="+bugconfigid;
    	//alert(urlStr);
    	var result = $.ajax({
			url:urlStr,
			type:"post",
	...
	*/
	function delversion(objj){
		...
		var pid=<s:property value="projectId" />;
		var urlStr=("<%=rootPath%>bugconfig/delversion.action?projectId="+pid+"&bugconfigid="+bugconfigid).replace(" ","");
		//alert(urlStr);
	...
	*/
	function delmodul(objj){
		...
		var pid=<s:property value="projectId" />;
		var urlStr=("<%=rootPath%>bugconfig/delmodul.action?projectId="+pid+"&bugconfigid="+bugconfigid).replace(" ","");
  ...
  <div >
	...
	<input  type="text"  name="versionname"  id="modulname"/>
