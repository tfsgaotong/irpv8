...
<script type="text/javascript" src="<%=rootPath %>client/js/jquery-ui.min.js"></script>

<script type="text/javascript"> 
var isdoc=<s:property value="isqusertionordoc"/>;
  //此处需要让其动态的生成一个table并填充数据 
function createTable(){
	 var imgStr = "<td  align=center bgcolor=#F5FAFE class=main_bleft>是否封面</td>";
	 if(isdoc==2){
	 imgStr = "";
	 }
	  ...
	  tableStr = tableStr + "<thead><tr><td align=center bgcolor=#F5FAFE class=main_bleft>原文件名称</td><td  align=center bgcolor=#F5FAFE class=main_bleft>显示名称</td>"+imgStr+"<td  align=center bgcolor=#F5FAFE class=main_bleft>排序</td><td   align='center' bgcolor=#F5FAFE class=main_bleft>操作</td></tr></thead>"; 
	  ...
	  for(var i=0 ;i<len ; i++){ 
	    ...
	    if(_allattacheds[i].editversions=="2"){//一种就是附件，
			if(isdoc==2){
			tableStr = tableStr + "<tr><td name='"+_allattacheds[i].attfile+"'  style=\"cursor:pointer; margin: 0 3px;\" onmouseout=\"hidepic()\" onmouseover=\"showpic('"+_allattacheds[i].attfile+"')\"   bgcolor=#F5FAFE class=main_bleft>"+ _allattacheds[i].attdesc +"</td><td   bgcolor=#F5FAFE class=main_bleft>"+_allattacheds[i].attlinkalt+"</td>";
			}else{
			if(_allattacheds[i].typeid=="2"){
			tableStr = tableStr + "<tr><td name='"+_allattacheds[i].attfile+"'  style=\"cursor:pointer; margin: 0 3px;\" onmouseout=\"hidepic()\" onmouseover=\"showpic('"+_allattacheds[i].attfile+"')\"   bgcolor=#F5FAFE class=main_bleft>"+ _allattacheds[i].attdesc +"</td><td   bgcolor=#F5FAFE class=main_bleft>"+_allattacheds[i].attlinkalt+"</td><td align=center bgcolor=#F5FAFE class=main_bleft><input type='radio' value=1 name='editversions'  id='"+_allattacheds[i].attfile+"'></td>";
			}else{
	    	        ...
	    	        $('#uploadify').uploadify('upload');
		   	       ...
		   	       if(isdoc!=2){
		   	        ...
		   	        var  tablerowAmount=$('#updateatttab').find('tr');
	   	         	 rowtr=rowtr+"<td align='center' bgcolor=#F5FAFE class=main_bleft><input   onkeyup='updateorderby(this)' onbeforepaste='updateorderby(this)' value='"+tablerowAmount.length+"'></td>";
	   	       		//操作
	   	      		rowtr=rowtr+"<td align='center' bgcolor=#F5FAFE class=main_bleft>";
	   	      		//添加操作图片
