 ...
 */
	function checkOnly(){
 	...
 	function termlinklist(_termid){
   	 	window.open('<%=rootPath %>term/termlinklist.action?refrechstatus=1&termid='+_termid); 
   	} 
	...
	$("input[name='chnlDocLinkscatach']").each(function(i){
		...
		$('#listSearchText').searchbox({
		    ...
		    searcher:function(value,name){
		    	...
		    	$('#selecQuestionQuotecontent').panel('refresh',"<%=rootPath %>site/passAuditTerm.action?"+queryString);
		    },   
		    menu:'#listSearchType',   
		    prompt:'请输入检索词'  
		});
...
<div id="listSearchType" style="width:120px;">  
    <div data-options="name:'termname'">词条标题</div>
    <div data-options="name:'cruserid'">创建者</div>
</div>
...
<table width="100%" border="0" align="center" cellpadding="10"
		...
		<tr>
		 ...
		 <td align="center" bgcolor="#F5FAFE" class="main_bright" width="62%"><a href="javascript:void(0)" onclick="orderBy('termname','<s:if test="orderField=='termname'&&orderBy=='desc'">asc</s:if><s:else>desc</s:else>')">词条标题<s:if test="orderField=='termname'&&orderBy=='desc'"><img src="<%=rootPath %>admin/images/spinner_arrow_down.gif" /></s:if><s:elseif test="orderField=='termname'&&orderBy=='asc'"><img src="<%=rootPath %>admin/images/spinner_arrow_up.gif" /></s:elseif></a></td>
		...
		<s:iterator value="termlist" status="termliststatus">
		<tr>
		 <td align="center" bgcolor="#F5FAFE" class="main_bright" width="10%"><input id="selecdocid<s:property value='termid' />" name="chnlDocLinkscatach" onclick="moveSelectAlready(<s:property value='termid' />)" type="radio" value="<s:property value='termid' />">&nbsp;<s:property value="(pageNum-1)*pageSize+#termliststatus.count"/></td>
		 <td align="left" bgcolor="#F5FAFE" class="main_bright" style="padding-left: 10px"  ><a href="javascript:void(0)" onclick="termlinklist(<s:property value='termid'/>)" title="<s:property value='termname'/>"><s:property value='termname'/></a></td>
		 ...
		 <td align="center" bgcolor="#F5FAFE" class="main_bright"  ><s:property value='etShowPageViewNameByUserId(cruserid)'/></td>
		</tr>
		</s:iterator>
