 ...
 <s:include value="../include/enterprise_head.jsp"></s:include> 
    ...
    function  probabledocumentshow(querystring){
    	...
    	if(querystring!="" && querystring!= undefined){
    		   str=$.ajax({
 				type:'post',
 				url:"<%=rootPath%>site/subprobabledocumentshow.action?"+querystring, 
 				dataType: "json",
 				async: false,
 			    cache: false 
 				}).responseText;  
  ...
  function updatePageHtml(_sFiled,_sOrderBy){
	...
	function page(_nPageNum){ 
    	var typenum = $('#searchorder option:selected').val();
		$('#pageForm').find('input[name="pageNum"]').val(_nPageNum);
		var queryString=$('#pageForm').serialize(); 
		queryString=queryString+"&searchsort="+typenum;
	...
	$(function(){ 
		var setting = {
			...
			async: {
				enable: true,
				url:"<%=rootPath%>site/docSubMapZtree.action" 
			},
			  callback: {
				onClick: oncClick
			} 
		...
		//获取树节点的第一个节点id
		$.ajax({
			url: '<%=rootPath%>site/getFirstNode.action',
   			type:'post', 
   		    async: false ,
   		    cache: false ,
   		    success:function(mes){
   		    	if(mes!=0){
   		    		str=$.ajax({
   		 				type:'post',
   		 				url:'<%=rootPath%>site/subprobabledocumentshow.action?id='+mes, 
   		 				dataType: "json",
   		 				async: false,
   		 			    cache: false 
   		 				}).responseText;  
    	 ...
    	 $.dialog({
    			...
    			ok: function(){
    				...
    				$('#createsubjectform').form('submit',{
    	    			url : "<%=rootPath %>site/begincreateqiyesubject.action?id="+id,
    	    			success:function(data){
    	    				if(data=='1'){
    	    					$.dialog.tips('创建成功',1,'32X32/succ.png',function(){
    	    						window.location="<%=rootPath %>site/subdocumentmaplistshow.action";
    	    					});
    	    				}else if(data=='2'){
