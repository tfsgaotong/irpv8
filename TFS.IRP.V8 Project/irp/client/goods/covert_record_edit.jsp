			    ...
			    <ul class="myDiscuss list9" id="<s:property value='usergoodsid' />div" >
			   		<li><div style="padding-left: 40px"><s:date name="coverttime" format="yyyy-MM-dd HH:mm" /><div class="userIcon" >
						         ...
						         <s:if test="findIrpUserByFocusId(userid).userpic!=null">
						          <img style="border-radius: 20px;" width="60" height="60" src="<%=rootPath%>file/readfile.action?fileName=<s:property value='findIrpUserByFocusId(userid).userpic' />" onmouseover="personalCard(<s:property value='userid' />,<s:property value='#resverobj.messageid' />)" onmouseout="personalCardOut(<s:property value='#resverobj.messageid' />)"   >
						         </s:if>
						         <s:elseif test="findIrpUserByFocusId(userid).userpic==null"> 
						          <img style="border-radius: 20px;" width="60" height="60" <s:if test="findIrpUserByFocusId(userid).sex==2">src="<%=rootPath%>view/images/female.jpg"</s:if><s:else>src="<%=rootPath%>view/images/male.jpg"</s:else> alt="用户图片"   onmouseover="personalCard(<s:property value='userid' />,<s:property value='#resverobj.messageid' />)" onmouseout="personalCardOut(<s:property value='#resverobj.messageid' />)"  />
						         </s:elseif>
