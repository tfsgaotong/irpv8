...
function editBug(){ 
	...
	if(isSubmit){
		...
		var saveUrl="<%=rootPath %>phone/updatebug.action?"+postStr;
		$.ajax({
			type:'post',
			url:saveUrl,
			async: true,
...
<form id="addBugPage"  method="post" onsubmit="return false;">
	 	...
	 	<div  class="newForm" style="width: 100%;float: none;margin-top: 30px;"> 
			<div> 
				...
				&nbsp;&nbsp;<input style="width: 140px" class="artTitle easyui-validatebox" required="true" validtype="length[2,300]" placeholder="添加Bug标题"  id="bugTitle"  value="<s:property value='irpBugs.get(0).title'/>" />  
			</div> 
        </div>
        <div style="float: left;width: 100%;">&nbsp;</div><br/>
        ...
        <div class="newForm"  style="width: 100%;float: none;margin-top: 20px;"> 
        	...
        	&nbsp;&nbsp; 
				<input class="easyui-validatebox" required="true" id="offPersonName" readonly="readonly" onclick="checkTaskOffpersonId()"  fit=“true”    type="text"  value="<s:property value="operatorNameMap.get(irpBugs.get(irpBugs.size()-1).founderid)" />"/> 
