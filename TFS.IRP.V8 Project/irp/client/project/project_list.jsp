...
<style>
 ...
 .listQz01 li .infoQz .lineUp .editBar .ends{display:inline-block; width:100px; height:24px; line-height:24px; color:#fff; background:url(<%=rootPath %>client/images/project/icon-n-05.png) no-repeat 7px 9px #909090; padding-left:15px; border-radius:30px; overflow:hidden; font-size:12px; cursor:pointer; margin-top:5px; margin-left:30px; float:left;}
.listQz01 li .infoQz .lineUp .editBar .ends:hover{background-color:#48b0eb;}
</style>
<script>
	//分页
                    	...
                    	<div class="lineUp">
                        	<div class="editBar">
                                <s:if test="loginUserid==offpersonid"> 
                           		<a class="del"  id="deleteprojecta<s:property value='projectid'/>" onclick="deleteproject(<s:property value='projectid'/>)"  href="javascript:void(0);"></a>
                                <s:if test="isclosed==@com.tfs.irp.project.entity.IrpProject@NOT_CLOSED">
                               	   <a class="end" style="padding: 2px 0px;" id="closeprojecta<s:property value='projectid'/>" href="javascript:void(0);" >
                               ...
                               <%--  <span class="end" style="padding: 3px 0px;">结束圈子</span> --%>
                                <script type="text/javascript">
                          		$(function(){
                          			...
                          			if(_personState==_irpProjectPersonStatePerson && _offpersonid!=_loginUserid){//说明是成员
                          				var msg='<b style="margin-left: 27px;">正在参与</b>';
                          				...
                          				var msg='<b style="margin-left: 27px;">正在关注</b>';
                          				...
                          				if(projectIsPub==_static_isPub){//圈子是私密圈子
                          					var msg='<b style="margin-left: 27px;">私密圈子</b>';
                          					$('#simi'+_projectId).addClass('ends');
	                          				$('#simi'+_projectId).html(msg);
	                          				$('#deleteprojecta'+_projectId).hide();
	                          				$('#closeprojecta'+_projectId).hide();	
                          				}else{ 
	                          				var msg='<b style="margin-left: 27px;">关注圈子</b>';
	                          				$('#attention'+_projectId).addClass('ends');
	                          				$('#attention'+_projectId).html(msg);
	                          				$('#deleteprojecta'+_projectId).hide();
	                          				$('#closeprojecta'+_projectId).hide();	
