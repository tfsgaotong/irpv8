...
<div class="labs" <s:if test="isXiangGuan=='true'">onmouseover="mouseEvt(<s:property value='recommendid'/>,1)" onmouseout="mouseEvt(<s:property value='recommendid'/>,0)"</s:if>>
	<dl>
    	<dt><%-- <a href="<s:url namespace="/site" action="client_to_index_person"><s:param name="personid" value="cruserid" /></s:url>" target="_blank"></a> --%><a><img src="<s:property value='userPicUrl'/>"/></a></dt>
        <dd class="text" style="width: 75%">
        	<div class="user">
        		<%-- <a href="<s:url namespace="/site" action="client_to_index_person"><s:param name="personid" value="cruserid" /></s:url>" target="_blank"> --%><a></a><s:property value="#userName"/></a>
        		<aside>
        			<s:if test="isXiangGuan=='true'">
        			<a href="#discuss" id="replyLink_<s:property value='recommendid'/>" style="display:none;"
        			  onclick="appendReplayUsername(<s:property value='recommendid'/>)">[回复]</a></s:if>
