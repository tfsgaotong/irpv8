...
<head>
<meta charset="utf-8">
<title>知识分类-<s:property value="irpChannel.chnldesc" /></title>
...
<body>
<form id="pageForm" action="<s:url namespace="/document" action="document_list"><s:param name="channelid" value="channelid" /></s:url>" method="post">
	...
	<nav class="privateNav">
    	<dl>
            ...
            <dd class="leftBox navBtns">
            	<a href="<s:url namespace="/document" action="document_list" />" class="current">知识分类</a>
...
<section class="mainBox">
	...
	<s:iterator value="rootChannels" status="index">
		...
		<div class="columnInfo" style="float:right;padding:3px 2px 0 0 ;">
						<input type="button" style="width:135px;height:35px;line-height:25px;font-size:15px;padding-left:20px;background-position:6px;" class="create" onclick="window.open('<s:url namespace="/document" action="document_edit"><s:param name="id" value="channelid" /></s:url>');" value="创建知识">
		</div>
	</article>
        	...
        	<div class="folder">
                ...
                <section>
                	...
                	<s:iterator value="#childChnls">
                    	<li><a href="<s:url namespace="/document" action="document_list"><s:param name="channelid" value="channelid" /></s:url>" title="<s:property value="chnldesc" />"><s:property value="chnldesc" /></a></li>
                    </s:iterator>
                    </ul>
                </section>
                </s:if>
    ...
    <section class="result">
    	<div class="title">
    		精华知识
    		<em onclick="window.location.href='<s:url namespace="/document" action="document_essence_list"><s:param name="channelid" value="channelid" /></s:url>'">更多</em>
   		</div>
    	<section class="mavin">
        	<dl>
        	<s:iterator value="essenceDocs">
        		<s:set var="document" value="getIrpDocumentById(docid)" />
        		<dt>
        			<a href="<s:url action="document_detail" namespace="/document"><s:param name="docid" value="docid" /></s:url>" title="<s:property value='doctitle'/>" target="_blank"><img src="<s:property value='docCoverPath(docid,docflag)' />" width="132" height="86" /></a>
        			<article>
        				...
        				<p style="height:55px;" class="ellipsis" title="<s:property value="#document.docabstract" />"><s:property value="#document.docabstract" /></p>
        			</article>
        		</dt>
        	</s:iterator>
            </dl>
        ...
        <section id="list" class="resultList" style="display:none;">
        	<div class="reorder">
        		<span>显示方式：</span><em class="current" onclick="tabView('list')">列表</em><em onclick="tabView('view')">视图</em>
		...
		<section id="view" class="resultList" style="display:none;">
        	<div class="reorder">
        		<span>显示方式：</span><em onclick="tabView('list')">列表</em><em onclick="tabView('view')" class="current">视图</em>
        		<div class="columnInfo" style="float:right;padding:2px 10px 0 0 ;">
					<input type="button" style="width:100px;height:25px;line-height:25px;font-size:15px;padding-left:20px;background-position:6px;" class="create" onclick="window.open('<s:url namespace="/document" action="document_edit"><s:param name="id" value="channelid" /></s:url>');" value="创建知识">
				</div>
        	</div>
		<s:if test="chnlDocLinks!=null && chnlDocLinks.size()>0">
			<s:iterator value="chnlDocLinks">
