...
d=b.getBogus(),c;d&&d.remove();
c=b.getHtml();
//console.log("bbbbb"+c);
var file=b.getHtml();
	var img = file.match(/file:\/\/+(localhost)?(\S+\.(png|jpg|jpeg|gif|bmp))/i);
	 console.log("iiiiiiiiiiiiiiii"+img);
	var file1= file.split(img);
	for(var i in file1){
		console.log("22222"+file1[i]);
		img = file1[i].match(/file:\/\/+(localhost)?(\S+\.(png|jpg|jpeg|gif|bmp))/i);
		console.log("33333333"+img);
	}
