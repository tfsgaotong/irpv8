...
EDiaryEditor = {
	...
	initView: function (sParentID) {
		var _this = this;
		var _toolbarStr = "\
			<div id='EDiaryEditor'style='width:800px; padding-left:2px;'>\
				<div id='EDiaryEditorToolBar' style='position: relative; width: 860px; height: 69px; background: url(images/editor/toolbar_bg.gif) -72;'>\
		...
		var _toolbarStr5 = "\
			<div style='position: absolute; background-image: url(images/editor/blank.gif); left: 0px; top: 0px; width: " + $("EDiaryEditor").offsetWidth + "px; height: " + $("EDiaryEditor").offsetHeight + "px; display: none;' id='EDiaryEditorDialog'>\
				<div style='position: absolute; background-image: url(images/editor/dilog_bg.gif); left: 160px; top: 140px; width: 780px; height: 157px; display: none;' id='EDiaryEditorIMGDialog'>\
	...
	initControl: function (bWordMode, sContent) {
		...
		function viewDialog(el, Act) {
			...
			if(window._isIE) {
				...
				if(Act == "img") {
					win = window.open("editor/img.htm?version=200712142", null, "Width=400,Height=200");
...
//			}
//		}
		$("EDiaryEditorface").onclick = $("EDiaryEditorhilitecolor").onclick = $("EDiaryEditorforecolor").onclick = $("EDiaryEditorFontStyleMenu").onclick = $("EDiaryEditorFontSizeMenu").onclick = $("EDiaryEditorFontFamilyMenu").onclick = function (event) {
			FontMenuFuncHash.click(event, FontMenuFuncHash);
			event = event || window.event;
			if(window._isIE){
				event.cancelBubble = true;
		...
		<script language=\"JavaScript\">\
			function resizeImg(_oObj, _iWidth){\
				...
				if(tMark){if(_iWidth){setTimeout(\"resize()\",750);}}\
	...
	runCMD: function (CMD, sValue) {
		...
		if(CMD == "" || CMD == null)return;
			...
			var html = "<a href=\"javascript:void(0)\" onclick=\"searchInfoWord('"+rng.text+"')\" >" + rng.text  + "</a>";
			if(window._isIE) {
	...
	insertFace: function (nFaceNum) {
		...
		var FacePath = "/"+location[3]+"/editor/neweditor/face/";
		this.iframe.contentWindow.focus();   
