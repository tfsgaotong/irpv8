        ...
        $.post('<%=rootPath%>asseroomapply/asseroomapplylist.action',date, function(data) {
			...
			//某个日期，会议数量 
			 ...
			 if(i>=0 && i<=6){  
			 		...
			 		$(clsstr).append("<div class=\"cls_gt_dis\" style=\"position:absolute;top:338px;width:14%;text-align:center;cursor:pointer;\" onclick=\"linkRC('"+data_s+"')\">会议:"+huinum+"，详情</div>");
			 	}else if(i>=21 && i<=27){
			 	
			 		$(clsstr).append("<div class=\"cls_gt_dis\" style=\"position:absolute;top:448px;width:14%;text-align:center;cursor:pointer;\" onclick=\"linkRC('"+data_s+"')\">会议:"+huinum+"，详情</div>");
			 	}else if(i>=28 && i<=34){
			 	
			 		$(clsstr).append("<div class=\"cls_gt_dis\" style=\"position:absolute;top:563px;width:14%;text-align:center;cursor:pointer;\" onclick=\"linkRC('"+data_s+"')\">会议:"+huinum+"，详情</div>");
			 	}else if(i>=35 && i<=41){
			 	
			 		$(clsstr).append("<div class=\"cls_gt_dis\" style=\"position:absolute;top:678px;width:14%;text-align:center;cursor:pointer;\" onclick=\"linkRC('"+data_s+"')\">会议:"+huinum+"，详情</div>");
			 	} 
 			}  
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		//goto date function
		if ($.browser.msie) {
		    $("#calendar .fc-header-right table td:eq(0)").before('<td><div class="ui-state-default ui-corner-left ui-corner-right" style="border-right:0px;padding:1px 3px 2px;" ><input type="text" id="selecteddate" size="10" style="padding:0px;z-index:1"></div></td><td><div class="ui-state-default ui-corner-left ui-corner-right"><a><span id="selectdate" class="ui-icon ui-icon-search">goto</span></a></div></td><td><span class="fc-header-space"></span></td>');
		} else {
		    $("#calendar .fc-header-right table td:eq(0)").before('<td><div class="ui-state-default ui-corner-left ui-corner-right" style="border-right:0px;padding:3px 2px 4px;" ><input type="text" id="selecteddate" size="10" style="padding:0px;z-index:1"></div></td><td><div class="ui-state-default ui-corner-left ui-corner-right"><a><span id="selectdate" class="ui-icon ui-icon-search">goto</span></a></div></td><td><span class="fc-header-space"></span></td>');
		}
		$("#selecteddate").datepicker({
			...
			beforeShow : function(input, instant) {
				setTimeout(function() {$('#ui-datepicker-div').css("z-index", "");}, 100);
			}
		});
...
/* 					for(var i = 0;i<=41;i++){ 
						var clsstr = ".fc-day"+i;
				 		$(clsstr).css("background-color","none");
				...
				eventAfterRender: function (event, element, view) {
				    ...
				    if (view.name == "month") {//按月份
				        ...
				        evtcontent = evtcontent + '<span class="fc-event-titlebg" style="width:115px;height:15px; white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + fstart + " - " + fend + '</span>';
	...
	function sqOneSel(_startDate,_endDate){
				...
				var str=$.ajax({//去后台拿取会议室预约索要用到的信息
					type:'post',
					dataType: "json",	     
					url:'<%=rootPath%>asseroomapply/toaddroot.action',
					data:{
						searchWord:asseroomid,start:starttime,end:endtime,warn:warntime,preOrOld:1,
					},
					async: false,
					cache: false
				}).responseText; 
						$.dialog({
							...
							$.ajax({
								type: "POST", //使用post方法访问后台
								url: "<%=rootPath %>asseroomapply/rootoneapply.action", //要访问的后台地址
								data: submit, //要发送的数据
								success: function (data) {
									if(data==2){
