...
$(function(){
    	...
    	if(focusonfusval==1){
    		...
    		$.ajax({
    			...
    			success:function(callback){
    				$(".mainBox").find(".weibo").append(callback);
    				$(".mainBox").find(".infomation").height($(".mainBox").find(".weibo").height());
    			}
    		});
	...
	*/
	...
	function AllCollection_wb(_pagenum){
	    var sHtmlConn = $.ajax({ 
	 		type:'post',
	 		data:'personid=<s:property value="personid"/>', 
	 		url: '<%=rootPath%>microblog/collectMicroblogListByPage.action',
	 		data:{
	    		pageNum:_pagenum,
	    		personid:<s:property value="personid"/>
	    	},
	 		dataType: "json",
			async: false,
		    cache: false  
	...
	//投稿
	function allsubmission(_pageNum){
	    var sHtmlConn = $.ajax({ 
	    	type:'post', 
	 		url: '<%=rootPath%>site/myallsubmission.action',
	 		data: 'personid=<s:property value='personid'/>&crtimesort=desc&pageNum='+_pageNum,
	 		dataType: "json",
			async: false,
		    cache: false  
	 	}).responseText;
					...
					onSubmit:function(){
						...
						if(commentlimits!="false"){
							if($("input[type='checkbox']").is(':checked')){
						    	...
						    	var content_1 = $.trim(trancontent);
...
function findMydocSubscribe(){
		var sUrl = "<%=rootPath%>subscribe/selectSubscribeDoc.action";
		var sHtmlConn = $.ajax({ 
	 		type:'post', 
	 		url: sUrl,
	 		dataType: "json",
		...
		$("#docSubscribe").html(sHtmlConn);
	}
	/*
	
	文章收藏
	*/
	function findMydocCollection(){
		var sUrl = "<%=rootPath%>site/mydoccollection.action";
		var sHtmlConn = $.ajax({ 
	 		type:'post', 
	 		url: sUrl,
	 		dataType: "json",
	...
	知识文章推荐
	*/
	function findMydocPush(){
		var sUrl = "<%=rootPath%>document/docPush.action";
		var sHtmlConn = $.ajax({ 
	 		type:'post', 
	 		url: sUrl,
	 		dataType: "json",
	...
	*/
	...
	function allMydocPush(_pagenum){
			var sUrl = "<%=rootPath%>document/docAllPush.action";
			var sHtmlConn = $.ajax({ 
			data:{
	    		pageNum:_pagenum
	    	},
	...
	*/
	...
	function allMydocSubscribe(_pagenum){
			var sUrl = "<%=rootPath%>subscribe/selectAllSubscribeDoc.action";
			var sHtmlConn = $.ajax({ 
			data:{
	    		pageNum:_pagenum
	    	},
	...
	所有文章收藏
	*/
	function allMydocCollection(_pagenum){
		var sUrl = "<%=rootPath%>site/myalldoccollection.action";
		var sHtmlConn = $.ajax({ 
		data:{
	    		pageNum:_pagenum
	    	},
...
<section class="mainBox" >
	<section class="weibo"  >
        ...
        <div class="screening" style="visibility: hidden;">
            ...
            <div>
            	<div id="tags" class="tags">
					...
					<a style="cursor:pointer;" onclick="findAllMicroblog();" >微知列表</a>
					<a style="cursor:pointer;" onclick="AllCollection_wb();" >微知收藏</a> 
					<a style="cursor:pointer;" onclick="allpiclist();">图片中心</a>
					 
					<!-- <a style="cursor:pointer;" onclick="allsubmission(1);">投稿</a> -->
					
					<s:if test="isCurrentUser==1">
						<a style="cursor:pointer;" onclick="allMydocPush(1);" >知识推荐</a>
						<a style="cursor:pointer;" onclick="allMydocSubscribe(1);" >文章订阅</a>
						<a style="cursor:pointer;" onclick="allMydocCollection(1);" >文章收藏</a>
					</s:if>
				</div>
			</div>
            <script>
        ...
        <div class="area20"></div>
      	...
      	<div  style="visibility: hidden;" class="labels">微知列表</div>
		 ...
		 <div  style="visibility: hidden;" class="labels">图片中心</div>
         <div  style="visibility: hidden;" class="line area20"></div>
	...
	<!-- 	<div style="visibility: hidden;" class="labels">投稿</div>
		...
		<s:if test="isCurrentUser==1">
			<div  style="visibility: hidden;" class="labels">知识推荐</div>
	      	<div  style="visibility: hidden;" class="line area20"></div>
	      	<div  style="visibility: hidden;" id="docPush"></div>
			<div  style="visibility: hidden;" class="labels">文章订阅</div>
	      	<div  style="visibility: hidden;" class="line area20"></div>
	      	<div  style="visibility: hidden;" id="docSubscribe"></div>
	      	<div  style="visibility: hidden;" class="labels">文章收藏</div>
	      	<div  style="visibility: hidden;" class="line area20"></div>
	      	<div  style="visibility: hidden;" id="docCollection"></div>
		</s:if>	
    ...
    <div class="OAForUser">
	    <input type="button" value="去签到" onclick="window.open('<s:url namespace="/sign" action="signInit" />');" class="create" style="width: 111px ; background-image: url('../view/images/sign_green.png');" />
	    <input class="create" style="width: 112px;margin-top: 2px; background-image: url('../view/images/sign_green.png');" type="button" value="请假" onclick="window.open('<s:url namespace="/leave" action="leaveList" ><s:param name="type" value="0"></s:param></s:url>');" />
	    <input class="create" style="width: 112px;margin-top: 2px; background-image: url('../view/images/sign_green.png');" type="button" value="加班" onclick="window.open('<s:url namespace="/leave" action="overtime_leaveList" />');" />
	    <input class="create" style="width: 345px;margin-top: 2px; background-image: url('../view/images/sign_green.png');" type="button" value="申请会议室" onclick="window.open('<s:url namespace="/asseroomapply" action="toAsseroomapply" />');" />
    </div>
    	...
    	<div class="userInfo">
        	<dl class="info">
                ...
                <dd class="leftBox">
                	...
                	<p>积分：<em><s:property value="sumscore" /></em></p>
                	 	...
                	 	<s:iterator value="medalimage.split(',')" var="attid"  status="index1">
		                  ...
		                  <div class="medal" >
		                   <a title="<s:property value='medalname' />:<s:property value="userCount" />" style="border-radius: 0px;height: auto;width: 60px;"href="<s:url action="medalDetial" namespace="/medal"><s:param name="medalid" value="medalid" /></s:url>"  target="_blank">
		                    <img src="<s:property value='coverPath(#attid)' />"  width="60" height="60" style="cursor: pointer;" title="<s:property value='medalname' />:<s:property value="userCount" />">
