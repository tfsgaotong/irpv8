...
font-family: "微软雅黑", "黑体";
  ...
  $(function(){
	...
	$.extend($.fn.validatebox.defaults.rules, {   
  	...
  	<form id="addprojectfrm" action="project/addproject.action" method="post">
 		...
 		<input type="hidden" name="irpProject.projectid"  value="<s:property value='irpProject.projectid'/>"/>
	  	<div>
		  	<font class='title'>圈子名称：</font>
	  	<font color="red">*</font>&nbsp;最多不超过20个字
		  	<input  style="background-color: rgb(246, 246, 246); line-height:20px;height: 30px;width: 430px"  class="easyui-validatebox txt_tclwbia" validType="length[0,20]"   required="true" id="prnameinput" name="irpProject.prname" value="<s:property value='irpProject.prname'/>"/>
	  	</div>
	  	<br>
	  	<div>
			<font class='title'>圈子描述：</font>   不超过100个字  <Br/>
			<textarea style="background-color: rgb(246, 246, 246);width: 435px"  required="true" id="prdescinput" name="irpProject.prdesc"   rows="" cols="6" class="easyui-validatebox description_clwbi" validType="length[0,100]"   ><s:property value="irpProject.prdesc"/></textarea>
	 	</div>
	...
	<br/>
	<font class='title'>圈子私密性：</font><select  name="irpProject.ispublish">
		...
		<option value="<s:property value='@com.tfs.irp.project.entity.IrpProject@NOT_PUBLISH'/>"  <s:if test="@com.tfs.irp.project.entity.IrpProject@NOT_PUBLISH==irpProject.ispublish">checked</s:if>>私密</option>
	   	     ...
	   	     &nbsp;&nbsp;   &nbsp;&nbsp;  	&nbsp;&nbsp;   &nbsp;&nbsp;   &nbsp;&nbsp;   &nbsp;&nbsp;   &nbsp;&nbsp;   
	<%-- <font class='title'> 圈子阶段：</font>
	  <select id="projectStatus"   name="irpProject.prostatus" class="easyui-combotree" style="width: 100px;" ></select>
	  <br/> --%>
	  <br/>
	<font class='title'>开始时间：</font><span id="starData">
			
				<input name="irpProject.starttime" type="text"  id="projectStart" style="width: 130px;" class="easyui-datebox" editable="false" validType="StartTime" value='<s:date name="irpProject.starttime" format="yyyy-MM-dd" />'>&nbsp;&nbsp; 
				</span>
	<font class='title'>结束时间：</font><span id="endData">
				
				<input name="irpProject.endtime" type="text" id="projectEnd" style="width: 130px;" class="easyui-datebox" editable="false" validType="EndTime" value='<s:date name="irpProject.endtime" format="yyyy-MM-dd"/>'>
				</span>
		<br/>
	<font class='title'>  圈子标签：</font>
	  <input class=" txt_tclwbia easyui-validatebox" style="background-color: rgb(246, 246, 246); margin-right: 10px;line-height:20px;height: 30px;width: 430px" validType="length[0,150]" id="keyprint" name="irpProject.prokey" value="<s:property value='irpProject.prokey'/>"/>
	 <span >
	 <br><br>
	 	<font class='title'>  圈子图标：</font> <span style="float: right; padding-right: 300px;padding-top:9px;"><a href="javascript:void(0);">
	 	<label id="projectImg">上传图标</label>
	 	  <input type="file" id="projectFileBut" />
