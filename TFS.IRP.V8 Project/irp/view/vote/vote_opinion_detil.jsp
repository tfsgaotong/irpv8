...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<%=rootPath%>view/css/votet.css"></link>
...
<script type="text/javascript">
 ...
 function checklook(){
	 ...
	 $.ajax({
		    ...
		    async: false,
		    cache: false,
			url:"<%=rootPath%>menu/vote_checkresultmore.action?voteid="+voteid,
		...
		$.dialog({
		    ...
		    ok: function(){
	    		...
	    		var microblog_text= mic;
	   			$.ajax({
	   				type:"POST",
	   				url:'<%=rootPath%>microblog/microblogSharepl.action',
	   				cache:false,
	   				data:{
	   					publishInfo:microblog_text,
	   					microblogType:microblog_type,
	   				...
	   				success:function(callback){
	   					if(callback!=null){
	   						...
	   						var urlvote="<%=rootPath%>menu/vote_detil.action?voteid="+vtid;
	   						window.location.href=urlvote;
	   					}else{
	   ...
	   var voteid='<s:property value="voteid" />';
		$.ajax({
			...
			success:function(callback){
				if(callback!=null){
					$.dialog.tips('发布成功',1,'32X32/succ.png');
					var urlvote="<%=rootPath%>menu/vote_detil.action?voteid="+voteid;
					window.location.href=urlvote;
				}else{
					$.dialog.tips('发布失败',1,'32X32/fail.png'); 
				}
